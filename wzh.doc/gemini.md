...
sequenceDiagram
    ...
    alt Namespace Invalid/Unavailable (during reload window)
        ...
        Note over Kubenswrapper: Log "Stale or mismatched namespace"
        Kubenswrapper ->>+ KubeletProcess: exec nsenter (without --mount) /usr/bin/hyperkube kubelet ...
        Note over KubeletProcess: Runs in WRONG namespace
        KubeletProcess -->> Kubenswrapper: Potential runtime errors (missing mounts, etc.)
        ...
        Note over KubeletProcess: Runs in CORRECT namespace
        KubeletProcess -->> Kubenswrapper: Service runs normally
    end
    
    Kubenswrapper -->>- KubeletService: Return status (success or failure)
    KubeletService -->>- Systemd: Service status (running or failed)
```
